...
@implementation TeslameterScopeView
...
- (id) initWithCoder:(NSCoder*)decoder
{
    ...
    if (self != nil) {
        self.rawDataX = malloc(TESLA_SCOPE_BUFFER_SIZE * sizeof(Float32));
        self.rawDataY = malloc(TESLA_SCOPE_BUFFER_SIZE * sizeof(Float32));
        self.rawDataZ = malloc(TESLA_SCOPE_BUFFER_SIZE * sizeof(Float32));
        ...
        _pinchScaleX = [[NSUserDefaults standardUserDefaults] floatForKey:kTeslaScopePinchScaleXKey];
        _pinchScaleY = [[NSUserDefaults standardUserDefaults] floatForKey:kTeslaScopePinchScaleYKey];
        _panOffset.x = [[NSUserDefaults standardUserDefaults] floatForKey:kTeslaScopePanOffsetXKey];
        _panOffset.y = [[NSUserDefaults standardUserDefaults] floatForKey:kTeslaScopePanOffsetYKey];
        if (_pinchScaleX < 1.0) {
            _pinchScaleX = 1.0;
            _panOffset.x = 0.0;
            _panOffset.y = 0.0;
...
- (IBAction) singleTapAction:(id) sender {
    ...
    if (((TeslameterScopeViewController *) _delegate).freeze) {
...
{
    CGRect frame = CGGraphRectMake(rect);
    BOOL isHideRuler = [[NSUserDefaults standardUserDefaults] boolForKey:kTeslaScopeHideRulerKey];
    Float32 volumeDecades = 10;
    Float32 org_x = frame.origin.x;
    Float32 org_y = frame.origin.y;
...
//    BOOL isHideRuler = [[NSUserDefaults standardUserDefaults] boolForKey:kAcceloScopeHideRulerKey];
    ...
    // リニア縦罫線
    CGContextBeginPath(context);
    NSUInteger times = TESLA_SCOPE_BUFFER_SIZE / 32.0;
...
//    CGContextSetFillColorWithColor(context, [UIColor colorWithRed:1.0 green:0.0 blue:0.0 alpha:0.4].CGColor);
    ...
    for (NSUInteger xyz = 0; xyz < 3; xyz++) {
        ...
        CGContextBeginPath(context);
        for (NSUInteger i = 0; i < TESLA_SCOPE_BUFFER_SIZE; i++) {
            Float32 fract;
            Float32 hpos;
            Float32 volumeOffset;
            Float32 rawMF = [self peekData:i xyz:xyz];
//            volumeOffset = height * rawMF / 100.0;    // 0μT〜100μT
            volumeOffset = height * rawMF / 500.0;    // 0μT〜500μT
            fract = (1.0 * i / ((Float32) TESLA_SCOPE_BUFFER_SIZE));
            if (i == 0) {
                hpos = org_x + 1.0;
...
- (IBAction) clearBuffer:(id)sender {
    ...
    [((TeslameterScopeViewController *) _delegate) initTeslameterScope];
}
...
- (void) enqueueData:(Float32) data xyz:(NSUInteger)xyz {
    @synchronized (self) {
        switch (xyz) {
            case 0:
                _rawDataX[_tailX++ % TESLA_SCOPE_BUFFER_SIZE] = data;
                break;
            case 1:
                _rawDataY[_tailY++ % TESLA_SCOPE_BUFFER_SIZE] = data;
                break;
            case 2:
                _rawDataZ[_tailZ++ % TESLA_SCOPE_BUFFER_SIZE] = data;
                break;
        }
    }
}
...
- (Float32) peekData:(NSInteger)index xyz:(NSUInteger)xyz {
    ...
    @synchronized (self) {
        switch (xyz) {
            case 0:
                if (_tailX < TESLA_SCOPE_BUFFER_SIZE) {
                    value = _rawDataX[index];
                } else {
                    value = _rawDataX[(index + _tailX + (TESLA_SCOPE_BUFFER_SIZE - 1)) % TESLA_SCOPE_BUFFER_SIZE];
                }
                break;
            case 1:
                if (_tailY < TESLA_SCOPE_BUFFER_SIZE) {
                    value = _rawDataY[index];
                } else {
                    value = _rawDataY[(index + _tailY + (TESLA_SCOPE_BUFFER_SIZE - 1)) % TESLA_SCOPE_BUFFER_SIZE];
                }
                break;
            case 2:
                if (_tailZ < TESLA_SCOPE_BUFFER_SIZE) {
                    value = _rawDataZ[index];
                } else {
                    value = _rawDataZ[(index + _tailZ + (TESLA_SCOPE_BUFFER_SIZE - 1)) % TESLA_SCOPE_BUFFER_SIZE];
                }
                break;
        }
    }
...
- (void) setDefaults {
    ...
    [[NSUserDefaults standardUserDefaults] setBool:NO forKey:kTeslaScopeHideRulerKey];
    [[NSUserDefaults standardUserDefaults] setFloat:_pinchScaleX forKey:kTeslaScopePinchScaleXKey];
    [[NSUserDefaults standardUserDefaults] setFloat:_pinchScaleY forKey:kTeslaScopePinchScaleYKey];
    [[NSUserDefaults standardUserDefaults] setFloat:_panOffset.x forKey:kTeslaScopePanOffsetXKey];
    [[NSUserDefaults standardUserDefaults] setFloat:_panOffset.y forKey:kTeslaScopePanOffsetYKey];
}
